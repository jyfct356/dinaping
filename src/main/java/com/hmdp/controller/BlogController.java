...
public class BlogController {
    ...
    public Result getBlogByUserId(@RequestParam(value = "current", defaultValue = "1") Integer current,
        ...
        List<Blog> blogs = blogService.query().eq("user_id", id)
                .page(new Page<>(current, SystemConstants.MAX_PAGE_SIZE)).getRecords();
        return Result.ok(blogs);
    }

    @GetMapping("/of/follow")
    public Result getBlogOfFollow(@RequestParam(value = "offset", defaultValue = "0") Long offset, @RequestParam(value = "lastId") Long lastId) {
        return blogService.getBlogOfFollow(offset, lastId);
    }
}
